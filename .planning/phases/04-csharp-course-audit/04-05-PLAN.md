...
files_modified:
  ...
  - content/courses/csharp/modules/**/challenges/**/*.cs (read + verify)
...
must_haves:
  truths:
    ...
    - "All challenge solution.cs files in M01-10 are syntactically valid C# top-level statements"
    - "All challenge.json files parse as valid JSON"
    - "All lesson.json files parse as valid JSON"
    - "ZERO stale .NET 8 references remain (except historical context)"
...
<task type="auto">
  ...
  <action>
    ...
    **Part B: Challenge Code Review (Modules 01-10)**
    ...
    4. Verify no C# 13 features are used in challenges where the Roslyn executor (4.8.0, C# 12 max) would reject them
       - Specifically check M05 lesson 06 (implicit index access), M06 lesson 08 (params collections), M10 lesson 05 (Lock type)
       - If these challenges use C# 13 features, add a comment at the top: `// NOTE: Requires C# 13 / .NET 9. May not execute in Roslyn 4.8.0 (C# 12 max).`
  ...
  <verify>
    ...
    - C# 13 challenges in M05/M06/M10 have Roslyn compatibility notes
    - Challenge solution.cs files in M01-10 use top-level statements
  </verify>
  <done>All JSON validated. Challenge code reviewed. Global sweep clean. Zero non-standard files, zero artifacts, minimal stale version references.</done>
</task>
...
- CSRP-03 partially satisfied: Challenge code reviewed, Roslyn execution limitations documented
