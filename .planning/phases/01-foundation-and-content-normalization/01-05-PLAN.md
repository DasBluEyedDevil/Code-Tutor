...
<task type="auto">
  ...
  <action>
    Read each course.json and add/update a `minimumRuntimeVersion` field that tells the app (and students) what minimum runtime is required to run the course's code.
    ...
    Also verify and update the `prerequisites` array if it contains version-specific information that is now outdated. For example, if a prerequisites entry says "Python 3.10+" update it to "Python 3.12+".

    IMPORTANT: The course.schema.json from Plan 02 allows additional properties, so adding `minimumRuntimeVersion` is schema-compatible. However, if Plan 02 has already run, update the course schema to explicitly include `minimumRuntimeVersion` as an optional string field.
  ...
  <done>All 6 course.json files include minimumRuntimeVersion. INFR-03 requirement (minimum runtime versions updated) is satisfied.</done>
</task>
