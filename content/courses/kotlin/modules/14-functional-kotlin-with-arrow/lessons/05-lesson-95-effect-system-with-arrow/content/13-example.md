...
class UserService(private val repository: UserRepository) {
    ...
    context(raise: Raise<UserServiceError>)
    suspend fun getUser(id: Long): User {
        raise.ensure(id > 0) { UserServiceError.ValidationFailed("Invalid ID: $id") }
        return repository.findById(id)
            ?: raise.raise(UserServiceError.NotFound(id))
    }

    context(raise: Raise<UserServiceError>)
    suspend fun createUser(name: String, email: String): User {
        raise.ensure(name.isNotBlank()) {
            UserServiceError.ValidationFailed("Name is required")
        }
        raise.ensure("@" in email) {
            UserServiceError.ValidationFailed("Invalid email")
        }

        val exists = repository.existsByEmail(email)
        raise.ensure(!exists) { UserServiceError.EmailConflict(email) }
    ...
    suspend fun updateEmail(userId: Long, newEmail: String): User {
        ...
        raise.ensure("@" in newEmail) {
            UserServiceError.ValidationFailed("Invalid email")
        }

        if (newEmail != user.email) {
            val exists = repository.existsByEmail(newEmail)
            raise.ensure(!exists) { UserServiceError.EmailConflict(newEmail) }
        }
