...
"@
    ...
    if (-not (Test-Path $IconPath)) {
        ...
        Write-Host "  ⚠ Icon file not found, installer will use default icon" -ForegroundColor Yellow
        # Use empty string for icon to use default
        $InnoScript = $InnoScript -replace "SetupIconFile=.*", "; SetupIconFile not specified"
        $InnoScript = $InnoScript -replace "UninstallDisplayIcon=.*", "; UninstallDisplayIcon not specified"
        $InnoScript | Out-File -FilePath $InnoScriptPath -Encoding UTF8 -Force
    }
